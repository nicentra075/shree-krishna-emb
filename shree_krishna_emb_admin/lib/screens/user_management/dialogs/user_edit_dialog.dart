...
class _UserEditDialogState extends State<UserEditDialog> {
  ...
  void initState() {
    ...
    _storeImageUrlController = TextEditingController(
      text: widget.user?.storeImageUrl ?? '',
    );
  ...
  bool _validateDesignerUrls(BuildContext context) {
    ...
    if (photo.isNotEmpty && !_isValidUrl(photo)) {
      ResponsiveSnackbar.showError(
        'Enter a valid Profile Image URL (http/https)',
        context,
      );
      return false;
    }
    if (storeImg.isNotEmpty && !_isValidUrl(storeImg)) {
      ResponsiveSnackbar.showError(
        'Enter a valid Store Image URL (http/https)',
        context,
      );
      return false;
    }
                          ...
                          if (isCreateMode) {
                            ...
                            context.read<UserListBloc>().add(
                              CreateUserEvent(
                                ...
                                storeName: _trimOrNull(
                                  _storeNameController.text,
                                ),
                                storeImageUrl: _trimOrNull(
                                  _storeImageUrlController.text,
                                ),
                                storeDescription: _trimOrNull(
                            ...
                            final updatedUser = UserListItemModel(
                              ...
                              storeDescription: isDesigner
                                  ...
                                  : null,
                              isAuthorisedSeller: isDesigner
                                  ? _isAuthorisedSeller
                                  : false,
                            );

                            context.read<UserListBloc>().add(
                              EditUserEvent(updatedUser),
                            );
                            Navigator.pop(context);
                          }
                        },
