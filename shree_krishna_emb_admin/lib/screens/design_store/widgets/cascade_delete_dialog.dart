    ...
    builder: (dialogCtx) {
      ...
      return StatefulBuilder(
        builder: (ctx, setState) {
          ...
          final note = !hasChildren ? null : (cascade ? cascadeNote : onlyNote);
          final noteColor = cascade
              ? const Color(0xFFD64545)
              : const Color(0xFFE6A23C);
          // Pin a stable width so the dialog doesn't resize when the note text
          // (and the cascade checkbox) changes between the two states.
          final double contentWidth = (MediaQuery.sizeOf(ctx).width - 80)
              .clamp(240.0, 420.0)
              .toDouble();
          ...
          return AlertDialog(
            ...
            title: Row(
              children: [
                Icon(Icons.warning_amber_rounded, color: noteColor, size: 22),
