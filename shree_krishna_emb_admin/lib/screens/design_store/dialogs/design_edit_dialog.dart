...
class _DesignEditDialogState extends State<DesignEditDialog> {
  ...
  void initState() {
    ...
    _discount = TextEditingController(text: (d?.discountAmount ?? 0).toString());
    _color = TextEditingController(text: d?.colorOrNeedleCount ?? '');
    _format = TextEditingController(text: d?.designFormat ?? '');
  ...
  Future<void> _loadAuthors() async {
    final result = await getIt<UserListRepository>()
  ...
  void dispose() {
    for (final c in [
      _name, _code, _author, _description, _price, _discount,
      _color, _format, _stitch, _height, _width,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    final categories =
        allCategories.where((c) => c.collectionId == _collectionId).toList();
      ...
      child: Container(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(isCreate ? strings.addDesign : strings.editDesign,
              ...
              AppTextField(label: strings.designName, hint: strings.designName, controller: _name),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: AppTextField(label: strings.code, hint: 'ABC-001', controller: _code)),
                const SizedBox(width: 12),
                Expanded(child: _authorPicker(strings)),
              ]),
              const SizedBox(height: 12),
              AppTextField(label: strings.descriptionLabel, hint: strings.descriptionLabel, controller: _description, maxLines: 2),
              const SizedBox(height: 12),
              ...
              SwitchListTile(
                ...
                title: Text(strings.isFree, style: AppTextStyles.labelMedium()),
                value: _isFree,
              ...
              if (!_isFree) ...[
                Row(children: [
                  Expanded(child: AppTextField(label: strings.price, hint: '0', controller: _price, keyboardType: TextInputType.number)),
                  const SizedBox(width: 12),
                  Expanded(child: AppTextField(label: strings.discountAmount, hint: '0', controller: _discount, keyboardType: TextInputType.number)),
                ]),
                const SizedBox(height: 8),
                Row(children: [
                  Text('${strings.finalPrice}: ',
              ...
              // Specs
              Row(children: [
                Expanded(child: AppTextField(label: strings.colorOrNeedleCount, hint: '9 needle', controller: _color)),
                const SizedBox(width: 12),
                Expanded(child: AppTextField(label: strings.designFormat, hint: 'DST', controller: _format)),
              ]),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: AppTextField(label: strings.stitchCount, hint: '0', controller: _stitch, keyboardType: TextInputType.number)),
                const SizedBox(width: 12),
                Expanded(child: AppTextField(label: strings.heightLabel, hint: '0', controller: _height, keyboardType: TextInputType.number)),
                const SizedBox(width: 12),
                Expanded(child: AppTextField(label: strings.widthLabel, hint: '0', controller: _width, keyboardType: TextInputType.number)),
              ]),
              const SizedBox(height: 12),
  ...
  Future<void> _save(BuildContext context, bool isCreate) async {
    ...
    final model = DesignModel(
      ...
      designFormat: _format.text.trim().isEmpty ? null : _format.text.trim(),
