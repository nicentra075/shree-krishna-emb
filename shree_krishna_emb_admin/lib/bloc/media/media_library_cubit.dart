...
class MediaLibraryState extends Equatable {
  ...
  @override
  List<Object?> get props => [status, assets, uploadingCount, error];
}
...
class MediaLibraryCubit extends Cubit<MediaLibraryState> {
  ...
  Future<String?> uploadMany(List<PendingUpload> uploads, int timestamp) async {
    ...
    emit(state.copyWith(uploadingCount: uploads.length));
    String? firstError;
    for (var i = 0; i < uploads.length; i++) {
      ...
      result.fold(
        (f) {
          ...
          emit(state.copyWith(uploadingCount: state.uploadingCount - 1));
        },
        (asset) => emit(state.copyWith(
    ...
    if (isClosed) return firstError;
    emit(state.copyWith(uploadingCount: 0, error: firstError));
    return firstError;
  }
